...
const loginRiskGuard = createStrategyRiskMiddleware({ action: 'login', blockThreshold: 90 });

router.post('/register', authWriteLimiter, authController.register);
router.post('/login', authLoginLimiter, loginRiskGuard, authController.login);
